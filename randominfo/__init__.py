...
full_path = lambda filename: abspath(join(getcwd(), dirname(__file__), filename))
...
def get_gender(first_name):
	gender_file = csv.reader(open(full_path('data.csv'), 'r'))
	gender = ""
	for data in gender_file:
...
def get_country(first_name = None):
	""" there are no countries in the file!!! i`ll generate random"""
	countries = ['United States', 'Canada', 'Mexico', 'Brazil', 'Argentina', 'Colombia', 'Peru', 'Venezuela', 'Chile',
				 'Ecuador']
	return choice(countries)
"""
	# if data file will be fixed - this code need to refactoring - set index 3 and 12 to index of countries in file
...
def get_email(person = None):
	...
	if c == 0:
		email = (person.first_name + get_formatted_datetime("%Y", person.birthdate, "%d %b, %Y")
				 + dmn + "." + ext)
	elif c == 1:
		email = (person.last_name + get_formatted_datetime("%d", person.birthdate, "%d %b, %Y")
				 + dmn + "." + ext)
	else:
		email = (person.first_name + get_formatted_datetime("%y", person.birthdate, "%d %b, %Y")
				 + dmn + "." + ext)
...
def get_alphabetic_profile_img(char, file_path, img_name, char_color = None, bg_color = None):
	...
	if all((c in chars) for c in img_name):
		...
		raise OSError("Invalid image name. Image name must contains character including digits, alphabets, white space, dot, comma, ( ) [ ] { } _ + - =.")
	return file_path
...
def get_address():
	full_addr = []
	# country field is not present in file, we will take it from Person class from attribute country
	...
	for i in range(4,10):
		...
		for addr in addr_file:
			try:
				if addr[i] != '':
					all_address.append(addr[i])
			except Exception as e:
				print(f"Somthing went wrong - {e}")
		full_addr.append(choice(all_address))
